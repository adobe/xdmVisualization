    ...
    JSV = {
        ...
        createPre: function(el, obj, title, exp) {
            ...
            var btn = $('<a href="#" class="ui-btn ui-mini ui-icon-action ui-btn-icon-right">Open in new window</a>').click(function() {
                ...
                w.document.title = title || 'XDM Schema Viewer';
                w.document.close();
            });
        ...
        compileData: function (schema, parent, name, real, depth) {
            ...
            node = {
                ...
                name: (schema.$ref && real ? name : false) || name || s.title || 'schema',
                isReal: real,
                plainName: name,
                type: s.type,
            ...
            if (parent) {
                if (node.name === 'item') {
                    node.parent = parent;
                    if(node.type) {
                        if (node.type === 'object') {node.name = s.id.split('/').slice(-1)[0].replace('.schema.json', '');}
            ...
            } else if(node.type === 'object' && node.name !== 'item') {
                node.name += ' { }';
            } else {
                node.name += ' ('+s.type+')';
            }
            ...
            for (key in props) {
                ...
                JSV.compileData(props[key],  node, key, true, depth + 1); //pass title for bug fixing rather then passing key
            }
    ...
    //                return d.children || d._children ? -10 : 10;
        ...
        createDiagram: function(callback) {
            tv4.asyncLoad([JSV.schema], function() {
                var schemaid = JSV.schema.split('/').slice(-1)[0].replace('.schema.json', '');
                JSV.compileData(tv4.getSchema(JSV.schema),false,schemaid);
